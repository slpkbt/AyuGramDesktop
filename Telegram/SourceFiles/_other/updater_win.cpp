		...
		do {
			...
			if (fname.substr(0, tdataDir.size()) == tdataDir && (fname.size() <= tdataDir.size() || fname.at(tdataDir.size()) == '/')) {
				...
				if (equal(tofname, updaterName)) { // bad update - has Updater.exe - delete all dir
					...
					delFolder();
					return false;
				} else if (equal(tofname, updateTo + L"SleepyGram.exe") && exeName != L"SleepyGram.exe") {
...
void updateRegistry() {
	if (versionNum && versionNum != 0x7FFFFFFF) {
		...
		HKEY rkey;
		LSTATUS status = RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{09F8EFBD-99A0-4326-9A2F-2694574B75CB}_is1", 0, KEY_QUERY_VALUE | KEY_SET_VALUE, &rkey);
					...
					if (PathCanonicalize(exp, locationStr)) {
						...
						if (GetFullPathName(L".", bufSize, exp, 0) < bufSize) {
							...
							if (installpath == mypath + L"\\" || true) { // always update reg info, if we found it
								...
								const WCHAR *appURL = L"https://github.com/slpkbt/AyuGramDesktop";
...
static const WCHAR *_programName = L"SleepyGram Desktop"; // folder in APPDATA, if current path is unavailable for writing
